...
const RitualTipHistoryList: React.FC = () => {
  ...
  const { history, isLoadingHistory, getRitualTipHistory } = useRitualTipStore(
    useShallow((state) => ({
      ...
      getRitualTipHistory: state.getRitualTipHistory,
    }))
  );
  ...
  const renderHistoryItem = ({ item }: { item: RitualHistoryItem }) => (
    <TouchableOpacity
      style={styles.historyCard}
      activeOpacity={0.7}
      onPress={() => {
        // Navigate to the Ritual Tip detail screen
        navigation.navigate('RitualTipHistoryDetail', { historyItem: item });
      }}
    >
      <Image
        source={ritualIcon}
      ...
      <View style={styles.cardTextContainer}>
        <Text style={styles.cardTitle}>RITUAL TIP</Text>
        <Text style={styles.cardSubtitle} numberOfLines={2}>
          {/* --- Displaying Ritual Name as requested --- */}
          {item.ritual_tip.ritual_name}
        </Text>
      </View>
      <View style={styles.cardRightContainer}>
        <Text style={styles.cardDate}>{formatDate(item.tip_date)}</Text>
        <GradientBox
...
export default RitualTipHistoryList;
