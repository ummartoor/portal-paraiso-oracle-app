    ...
    return DEFAULT_TABS.map(tab => {
      switch (tab.key) {
        case 'morning':
          return {
            ...tab,
            description: horoscopeData.horoscope.overview || '',
          };
        case 'career':
          return { ...tab, description: horoscopeData.horoscope.career || '' };
        case 'love':
          return { ...tab, description: horoscopeData.horoscope.love || '' };
        case 'money':
          return { ...tab, description: horoscopeData.horoscope.finance || '' };
        case 'health':
          return { ...tab, description: horoscopeData.horoscope.health || '' };
        case 'divine':
          return {
            ...tab,
            description: horoscopeData.horoscope.overview || '',
...
const AstrologyHistoryDetail: React.FC = () => {
  ...
  const route =
    useRoute<RouteProp<AppStackParamList, 'AstrologyHistoryDetail'>>();
  ...
  const horoscopeDataForTabs: HoroscopeData | null = useMemo(() => {
    if (!horoscopeItem || !horoscopeItem.horoscope) return null;
    // The InsightTabs component expects HoroscopeData with horoscope property
    return { horoscope: horoscopeItem.horoscope };
  }, [horoscopeItem]);
