 ...
 const { user, fetchCurrentUser } = useAuthStore();
  ...
  useEffect(() => {
    if (user?.sign_in_zodiac) {
      const userSignIndex = ZODIACS.findIndex(z => z.key === user.sign_in_zodiac);
      if (userSignIndex !== -1) {
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.calendarOuter}>
            <View style={styles.calendarWrap}>
              {days.map((item, idx) => {
                ...
                return (
                  <TouchableOpacity key={item.key} activeOpacity={0.9} onPress={() => setSelectedIdx(idx)} style={styles.calItem}>
                    <Text style={[styles.dayTop, { color: colors.white }]}>{item.day ?? ''}</Text>
                    {isSelected ? (
                      <GradientBox
                        colors={[colors.black, colors.bgBox]}
                        style={[styles.calBox, { borderColor: colors.primary , padding:0}]}
                      >
                        <Text style={[styles.dateText, { color: colors.white }]}>{String(item.dateNum ?? '')}</Text>
                      </GradientBox>
                    ) : (
                      <View style={[styles.calBox, { backgroundColor: colors.bgBox, borderColor: 'transparent' }]}>
                        <Text style={[styles.dateText, { color: colors.white }]}>{String(item.dateNum ?? '')}</Text>
                      </View>
                    )}
                  </TouchableOpacity>
          ...
          <View style={styles.zodiacRow}>
            ...
            <View style={styles.zodiacCenter}>
              <View style={[styles.zodiacImageFrame, { borderColor: 'rgba(255,255,255,0.12)' }]}>
                <Image source={current?.icon} style={styles.zodiacImage} resizeMode="contain" />
              </View>
          ...
          {isLoading ? (
            ...
            <View style={styles.errorContainer}>
              <Text style={styles.errorText}>Horoscope not available for this day.</Text>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  contentSubtitle: { marginTop: 6, fontFamily: Fonts.aeonikRegular, fontSize: 14, lineHeight: 18, opacity: 0.9 },
  calendarOuter: { marginTop: 25, width: '100%', alignItems: 'center' },
  calendarWrap: { width: CAL_WIDTH, flexDirection: 'row', justifyContent: 'space-between' },
  calItem: { alignItems: 'center' },
  calBox: { width: BOX_SIZE, height: BOX_SIZE, borderRadius: 8, alignItems: 'center', justifyContent: 'center', borderWidth: 1 },
  ...
  zodiacRow: { marginTop: 28, width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  sideCol: { width: 80 },
  sideLabel: { marginTop: 8, fontFamily: Fonts.cormorantSCBold, fontSize: 16, opacity: 0.8 },
  arrowWrap: { width: 31, height: 31, borderRadius: 30, alignItems: 'center', justifyContent: 'center', borderWidth: 1 },
  ...
  actionsRow: { marginTop: 24, width: '100%', flexDirection: 'row', justifyContent: 'space-between', gap: 12 },
  ...
  errorContainer: { marginTop: 30, width: '100%', paddingVertical: 40, alignItems: 'center', justifyContent: 'center' },
  errorText: { fontFamily: Fonts.aeonikRegular, fontSize: 16, color: 'rgba(255, 255, 255, 0.7)', textAlign: 'center' },
});
