...
                            if (index % 3 === 0) rows.push([card]); else rows[rows.length - 1].push(card); return rows;
                        }, []).map((row, rowIndex) => (
                            <View key={rowIndex} style={styles.selectedRow}>
                                {row.map(card => (
                                    <View key={card.card_id} style={styles.box}><Image source={{ uri: card.image.url }} style={styles.boxImg} /></View>
                                ))}
